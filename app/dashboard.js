...
function renderReport(r) { return '<div class="report-card"><strong>'+escapeHTML(r.summary||"-")+'</strong><div style="font-size:.85rem;color:var(--muted);margin-top:.25rem;">'+escapeHTML(r.hours||0)+' saat &middot; '+formatDate(r.reportDate)+' &middot; '+escapeHTML(r.status||"submitted")+(r.userEmail?' &middot; '+escapeHTML(r.userEmail):'')+'</div></div>'; }
...
async function loadHome() { document.getElementById("profileCard").innerHTML = renderProfile(currentProfile); var snap = await getDocs(query(collection(db,"announcements"),orderBy("createdAt","desc"),limit(3))); document.getElementById("homeAnnouncements").innerHTML = snap.empty?'<p class="empty">Duyuru yok.</p>':snap.docs.map(d=>renderAnnouncement(d.data())).join(""); }
async function loadTasks() { var isA=currentProfile.role==="admin"||currentProfile.role==="coordinator"; var q=isA?query(collection(db,"tasks"),orderBy("createdAt","desc"),limit(50)):query(collection(db,"tasks"),where("assignedToEmail","==",currentUser.email||""),orderBy("createdAt","desc"),limit(20)); var snap=await getDocs(q); document.getElementById("tasksList").innerHTML=snap.empty?'<p class="empty">Görev bulunamadı.</p>':snap.docs.map(d=>renderTask(d.data())).join(""); }
async function loadReports() { var isA=currentProfile.role==="admin"||currentProfile.role==="coordinator"; var q; if(isA){q=query(collection(db,"reports"),orderBy("createdAt","desc"),limit(50));document.getElementById("reportsTitle").textContent="Tüm raporlar";}else{q=query(collection(db,"reports"),where("userUid","==",currentUser.uid),orderBy("createdAt","desc"),limit(20));} var snap=await getDocs(q); document.getElementById("reportsList").innerHTML=snap.empty?'<p class="empty">Rapor bulunmuyor.</p>':snap.docs.map(d=>renderReport(d.data())).join(""); }
async function loadAnnouncements() { var snap=await getDocs(query(collection(db,"announcements"),orderBy("createdAt","desc"),limit(20))); document.getElementById("announcementsList").innerHTML=snap.empty?'<p class="empty">Duyuru yok.</p>':snap.docs.map(d=>renderAnnouncement(d.data())).join(""); }
async function loadPending() { var snap=await getDocs(query(collection(db,"users"),where("status","==","pending"),limit(30))); var el=document.getElementById("pendingUsers"); var pt=document.querySelector('[data-tab="pending"]'); if(snap.empty){el.innerHTML='<p class="empty">Bekleyen başvuru yok.</p>';return;} el.innerHTML=snap.docs.map(d=>renderPendingUser(d.data(),d.id)).join(""); if(pt){var ex=pt.querySelector(".count-badge");if(ex)ex.remove();pt.insertAdjacentHTML("beforeend",'<span class="count-badge">'+snap.size+'</span>');} }
async function loadUsers() { var snap=await getDocs(query(collection(db,"users"),orderBy("createdAt","desc"),limit(100))); document.getElementById("userDirectory").innerHTML=snap.empty?'<p class="empty">Kullanıcı bulunamadı.</p>':snap.docs.map(d=>renderUserRow(d.data(),d.id)).join(""); }

document.addEventListener("click", async function(e) {
  var ab=e.target.closest("button[data-action]"); if(ab&&db){var uid=ab.dataset.uid;var action=ab.dataset.action;try{ab.disabled=true;ab.textContent="...";await updateDoc(doc(db,"users",uid),{status:action==="approve"?"approved":"blocked",updatedAt:serverTimestamp()});var card=document.getElementById("user-"+uid);if(card){card.style.opacity="0.4";card.innerHTML='<div class="user-info"><strong>'+(action==="approve"?"Onaylandı":"Engellendi")+'</strong></div>';}}catch(err){console.error(err);alert("Hata: "+err.message);ab.disabled=false;}return;}
  var sb=e.target.closest("button[data-save-uid]"); if(sb&&db){var uid2=sb.dataset.saveUid;var row=document.getElementById("urow-"+uid2);var role=row.querySelector('[data-role-uid="'+uid2+'"]').value;var status=row.querySelector('[data-status-uid="'+uid2+'"]').value;try{sb.disabled=true;sb.textContent="...";await updateDoc(doc(db,"users",uid2),{role:role,status:status,updatedAt:serverTimestamp()});sb.textContent="OK";setTimeout(function(){sb.textContent="Kaydet";sb.disabled=false;},1500);}catch(err){console.error(err);alert("Hata: "+err.message);sb.textContent="Kaydet";sb.disabled=false;}}
});
...
document.getElementById("addUserForm")?.addEventListener("submit", async function(e) {
  ...
  try { await setDoc(doc(db,"preregistered",email),{email:email,fullName:document.getElementById("addUserName").value.trim(),department:document.getElementById("addUserDepartment").value,role:document.getElementById("addUserRole").value,phone:document.getElementById("addUserPhone").value.trim(),status:"approved",createdByUid:currentUser.uid,createdAt:serverTimestamp()});
    e.target.reset(); document.getElementById("addUserMessage").textContent="Ön kayıt oluşturuldu! Kişi giriş yaptığında otomatik onaylanacak."; setTimeout(function(){document.getElementById("addUserMessage").textContent=""},5000);
