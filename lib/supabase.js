...
export const getPublicStats = async () => {
  const result = await supabase.from('public_stats').select('*').single();
  return result;
};

export const getPublicAnnouncements = async () => {
  const result = await supabase.from('announcements').select('title, body, created_at')
    .eq('is_public', true)
...
const PROFILE_COLS = 'id,email,phone,display_name,avatar_url,role,department,bio,city,status,total_hours,joined_at,last_active,telegram_id,telegram_state,telegram_link_code,first_login,activity_score,activity_status,last_activity_at,is_backup_coordinator,admin_notes,tags,skills,available_days';

export const getProfile = (uid) =>
  supabase.from('profiles').select(PROFILE_COLS).eq('id', uid).single();

export const updateProfile = (uid, data) =>
  supabase.from('profiles').update(data).eq('id', uid).select(PROFILE_COLS).single();

export const getAllProfiles = () =>
  supabase.from('profiles').select(PROFILE_COLS).order('display_name');

export const getProfilesByRole = (role) =>
  supabase.from('profiles').select(PROFILE_COLS).eq('role', role).eq('status', 'active');

export const getProfilesByDept = (dept) =>
  supabase.from('profiles').select(PROFILE_COLS).eq('department', dept).eq('status', 'active');
...
export const getCoordsByDept = (dept) =>
  supabase.from('profiles').select(PROFILE_COLS).eq('department', dept).eq('role', 'coord').eq('status', 'active');
...
// ═══════════════════════════════════════════
// ÇALIŞMA RAPORLARI (yeni sistem)
// ═══════════════════════════════════════════

export const createWorkReport = (data) =>
  supabase.from('work_reports').insert(data).select().single();

export const getMyReports = (uid, limit = 20) =>
  supabase.from('work_reports').select('*').eq('user_id', uid).order('date', { ascending: false }).limit(limit);

export const getWeekReports = (uid) => {
  const now = new Date();
  const monday = new Date(now);
  monday.setDate(now.getDate() - ((now.getDay() + 6) % 7));
  return supabase.from('work_reports').select('*').eq('user_id', uid).gte('date', monday.toISOString().slice(0, 10)).order('date', { ascending: false });
};

export const updateWorkReport = (id, data) =>
  supabase.from('work_reports').update({ ...data, edited_at: new Date().toISOString() }).eq('id', id).select().single();

export const deleteWorkReport = (id) =>
  supabase.from('work_reports').delete().eq('id', id);

export const getPendingReports = () =>
  supabase.from('work_reports').select('*, profiles!user_id(display_name, department)')
    .eq('status', 'pending').order('date', { ascending: false });

export const approveReport = (id, reviewerId) =>
  supabase.from('work_reports').update({ status: 'approved', approved_by: reviewerId }).eq('id', id);

export const approveAllReports = async (ids, reviewerId) => {
  for (const id of ids) {
    await supabase.from('work_reports').update({ status: 'approved', approved_by: reviewerId }).eq('id', id);
  }
};

// ═══════════════════════════════════════════
// CHECK-IN / CHECK-OUT (eski — korunuyor)
// ═══════════════════════════════════════════

export const getActiveCheckin = (uid) =>
  supabase.from('checkins').select('*').eq('user_id', uid).eq('status', 'active').order('check_in', { ascending: false }).limit(1).maybeSingle();

export const checkIn = (uid, workMode = 'onsite') =>
  supabase.from('checkins').insert({ user_id: uid, check_in: new Date().toISOString(), date: new Date().toISOString().slice(0,10), work_mode: workMode }).select().single();

export const checkOut = (id, checkOutTime, workDone, nextPlan) => {
  const checkIn_t = new Date(checkOutTime);
  return supabase.from('checkins').update({
...
export const fixMissedCheckout = (id, checkOutTime) =>
  supabase.from('checkins').update({
    check_out: checkOutTime,
    status: 'completed',
    work_done: '(Gecikmiş çıkış)',
  }).eq('id', id);

export const getMyCheckins = (uid, limit = 10) =>
  supabase.from('checkins').select('*').eq('user_id', uid).neq('status', 'active').order('date', { ascending: false }).limit(limit);

export const getLastCheckin = (uid) =>
  supabase.from('checkins').select('*').eq('user_id', uid).eq('status', 'completed').order('date', { ascending: false }).limit(1).maybeSingle();

export const getWeekCheckins = (uid) => {
  const now = new Date();
  const monday = new Date(now);
  monday.setDate(now.getDate() - ((now.getDay() + 6) % 7));
  return supabase.from('checkins').select('*').eq('user_id', uid).gte('date', monday.toISOString().slice(0,10)).neq('status', 'active').order('date', { ascending: false });
};

export const getPendingCheckins = () =>
  supabase.from('checkins').select('*, profiles!user_id(display_name, department)')
    .eq('status', 'completed').order('date', { ascending: false });

export const getActiveCheckins = () =>
  supabase.from('checkins').select('*, profiles!user_id(display_name, department)')
    .eq('status', 'active').order('check_in', { ascending: true });

export const approveCheckin = (id, reviewerId) =>
  supabase.from('checkins').update({ status: 'approved', approved_by: reviewerId }).eq('id', id);

export const updateCheckinHours = async (id) => {
  const { data } = await supabase.from('checkins').select('check_in, check_out').eq('id', id).single();
  if (data?.check_in && data?.check_out) {
    const hours = Math.round((new Date(data.check_out) - new Date(data.check_in)) / 3600000 * 100) / 100;
    await supabase.from('checkins').update({ hours }).eq('id', id);
  }
};

export const addRetroactiveCheckin = async (uid, date, checkInTime, checkOutTime, workDone, source = 'web', workMode = 'onsite') => {
  const checkIn = `${date}T${checkInTime}:00`;
  const checkOut = `${date}T${checkOutTime}:00`;
  const hours = Math.round((new Date(checkOut) - new Date(checkIn)) / 3600000 * 100) / 100;
  return supabase.from('checkins').insert({
    user_id: uid, date, check_in: checkIn, check_out: checkOut, hours,
    work_done: workDone, status: 'completed', is_retroactive: true, source, work_mode: workMode,
  }).select().single();
};

export const hasCheckinOnDate = async (uid, date) => {
  const { data } = await supabase.from('checkins').select('id').eq('user_id', uid).eq('date', date).limit(1);
  return (data || []).length > 0;
};

export const getWorkSummary = (uid) =>
  supabase.from('volunteer_work_summary').select('*').eq('id', uid).single();

export const getAllWorkSummaries = () =>
  supabase.from('volunteer_work_summary').select('*').order('total_hours', { ascending: false });

// ═══════════════════════════════════════════
// SERTİFİKALAR
// ═══════════════════════════════════════════

export const getMyCertificates = (uid) =>
...
export const createCertificate = async (data) => {
  ...
  const num = (count || 0) + 1;
  const certNum = `TV-${year}-${String(num).padStart(4, '0')}`;
  return supabase.from('certificates').insert({ ...data, certificate_number: certNum }).select().single();
};
...
export const getReportArchive = (type, limit = 20) => {
...
export const verifyCertificate = (code) =>
  supabase.from('certificates').select('*, profiles!user_id(display_name)').or(`verify_code.eq.${code},certificate_number.eq.${code}`).single();

export const getCertificatesForUser = (uid) =>
  supabase.from('certificates').select('*').eq('user_id', uid).order('created_at', { ascending: false });

// ═══════════════════════════════════════════
// SAAT KAYITLARI (eski — korunuyor)
// ═══════════════════════════════════════════
...
export const getAllDataForBackup = async () => {
  const [profiles, tasks, hours, shifts, anns, apps, reqs, msgs, comments, progress, shiftNotes, notifs, vis, workReps] = await Promise.all([
    ...
    supabase.from('work_reports').select('*, profiles!user_id(display_name)').order('date', { ascending: false }),
  ]);
...
export const getRecentActivity = async () => {
  ...
  const items = [
    ...(hours.data || []).map(h => ({ type: 'hours', text: `${h.profiles?.display_name} ${h.hours} saat kaydetti (${h.department})`, time: h.created_at, icon: 'hours' })),
    ...(progress.data || []).map(p => ({ type: 'progress', text: `${p.profiles?.display_name} '${p.tasks?.title}' gorevini %${Math.round(p.new_value)}'e guncelledi`, time: p.created_at, icon: 'progress' })),
    ...(anns.data || []).map(a => ({ type: 'announcement', text: `Yeni duyuru: ${a.title}`, time: a.created_at, icon: 'announcement' })),
  ];
...
// ── New: recent reports, bulk notify, report data ──
export const getRecentReports = (limit = 10) =>
  supabase.from('work_reports').select('*, profiles!user_id(display_name)').order('created_at', { ascending: false }).limit(limit);

export const getReportsInRange = (start, end) =>
  supabase.from('work_reports').select('*, profiles!user_id(display_name, department)').gte('date', start).lte('date', end);

export const sendBulkNotification = async (userIds, type, title, body = '') => {
  const rows = userIds.map(id => ({ user_id: id, type, title, body }));
  return supabase.from('notifications').insert(rows);
};

export const getLastWeekHours = async () => {
  const now = new Date();
  const mon = new Date(now); mon.setDate(now.getDate() - ((now.getDay() + 6) % 7) - 7);
  const sun = new Date(mon); sun.setDate(mon.getDate() + 6);
  const { data } = await supabase.from('work_reports').select('hours').gte('date', mon.toISOString().slice(0,10)).lte('date', sun.toISOString().slice(0,10));
  return (data || []).reduce((a, r) => a + Number(r.hours || 0), 0);

};

export const subscribeReports = (callback) =>
  supabase.channel('work-reports-live')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'work_reports' }, callback)
    .subscribe();
...
export const upsertSiteContent = (key, content, userId) =>
  supabase.from('site_content').upsert({ key, content, updated_at: new Date().toISOString(), updated_by: userId }, { onConflict: 'key' });

// ── Public dashboard views ──
export const getPublicDashboard = async () => {
  const { data } = await supabase.from('public_dashboard').select('*').single();
  return data;
};
export const getPublicDeptWeekly = () =>
  supabase.from('public_dept_weekly').select('*');
export const getPublicTasksOverview = () =>
  supabase.from('public_tasks_overview').select('*');

// ── Admin actions ──
export const logAdminAction = (adminId, targetUserId, action, details = {}) =>
  supabase.from('admin_actions').insert({ admin_id: adminId, target_user_id: targetUserId, action, details });

export const getAdminActions = (limit = 20) =>
  supabase.from('admin_actions').select('*, profiles!admin_id(display_name), target:profiles!target_user_id(display_name)').order('created_at', { ascending: false }).limit(limit);

export const getUserReports = (userId, limit = 10) =>
  supabase.from('work_reports').select('*').eq('user_id', userId).order('date', { ascending: false }).limit(limit);

export const createReportForUser = (data) =>
  supabase.from('work_reports').insert(data).select().single();
