...
export const checkIn = (uid) =>
  supabase.from('checkins').insert({ user_id: uid, check_in: new Date().toISOString(), date: new Date().toISOString().slice(0,10) }).select().single();
...
export const addRetroactiveCheckin = async (uid, date, checkInTime, checkOutTime, workDone, source = 'web') => {
  ...
  return supabase.from('checkins').insert({
    ...
    work_done: workDone, status: 'completed', is_retroactive: true, source,
  }).select().single();
};
